...
function Users(props) {
    ...
    const handleExport = async (e) => {
        const res = await userApi.exportToCSV("USER", search, filter);
        // console.log(res);
        const blob = new Blob(["\uFEFF" + res], {
            type: "text/csv;charset=utf-8;",
        });
        if (filter === "") {
            fileDownload(blob, "ListStudent.csv");
        } else {
            fileDownload(blob, filter + ".csv");
        }
    };
            ...
            {showDeleteForm && !showUpdateForm && !showAddForm && (
                <div className={cx("delete-model")}>
                    <div className={cx("content")}>
                        <h3>XÁC NHẬN</h3>
                        <p>
                            Bạn có chắc chắn xóa sinh viên {userDelete.fullName}
                            <span>?</span>
                        </p>
                    </div>
                    <div className={cx("btn")}>
