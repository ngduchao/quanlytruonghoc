...
//			CustomSpecification teacherName = new CustomSpecification("teacherName", search);
		...
		if (!StringUtils.isEmpty(filter.getMajorCode())) {
...
class CustomSpecification implements Specification<Subject>{
	...
	public Predicate toPredicate(
		...
		if (field.equalsIgnoreCase("majorCode")) {
	        Join<Subject, Major> majorJoin = root.join("major");
	        return criteriaBuilder.equal(majorJoin.get("majorCode"), value.toString());
	    }
