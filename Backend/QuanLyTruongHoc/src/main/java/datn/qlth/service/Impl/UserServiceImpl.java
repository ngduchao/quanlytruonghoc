...
//		Specification<User> where = UserSpecification.buildWhere(search, filter);
//		Page<User> userPage = repository.findAll(where, pageable);
	...
	public void writeAccountToCsv(Writer writer, List<UserDTO> dtos) {
		try (CSVPrinter csvPrinter = new CSVPrinter(writer, CSVFormat.DEFAULT)) {
			csvPrinter.printRecord(CSVConstant.NO, CSVConstant.DISPLAY_NAME, CSVConstant.EMAIL_ADDRESS);

			for (UserDTO userEntity : dtos) {
				int index = dtos.indexOf(userEntity) + 1;

				csvPrinter.printRecord(index, userEntity.getUsername(), userEntity.getEmail());
			}
